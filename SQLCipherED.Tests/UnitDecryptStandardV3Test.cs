...
{
    public class UnitDecryptStandardV3Test
    {
        [Test]
        public void DecryptWithHexKey()
        {
            var reader = new BinaryReader(File.OpenRead("db-v3-hexkey.sqlcipher"));
            var key = Enumerable.Range(0, Constants.DbHexKeyStr.Length)
                            .Where(x => x % 2 == 0)
                            .Select(x => Convert.ToByte(Constants.DbHexKeyStr.Substring(x, 2), 16))
                            .ToArray();
            var decoder = new SQLCipherCryptEng(standard: SQLCipherStandard.V3);
            ...
            Assert.That(pageSize, Is.EqualTo(SQLCipherStandard.V3.PageSize()));
        }

        [Test]
        public void DecryptWithPassphrase()
        {
            var reader = new BinaryReader(File.OpenRead("db-v3-passphrase.sqlcipher"));
            var decoder = new SQLCipherCryptEng(standard: SQLCipherStandard.V3);
